function addActivity(actType, actName, actDescription) {
    ...
    var actDueDate = null;
    if (arguments.length > 3 && arguments[3]) thisUser = arguments[3];
    if (arguments.length > 4 && arguments[4]) itemCap = arguments[4];
    if (arguments.length > 5 && arguments[5]) thisInternalOnly = arguments[5];
    if (arguments.length > 6 && arguments[6]) actStatus = arguments[6];
    if (arguments.length > 7 && arguments[7]) actPriority = arguments[7];
    if (arguments.length > 8 && arguments[8]) actDueDate = arguments[8];
    ...
    if (pUser) myAct.setAssignedDeptNumber(pUser.getDeptOfUser());
    if (thisUser) myAct.setAssignedStaffID(thisUser);
    myAct.setInternalOnly(((thisInternalOnly && thisInternalOnly == "N") ? "N" : "Y"));
    if (actStatus) myAct.setActStatus(actStatus);
    if (actPriority) myAct.setPriority(actPriority);
    if (actDueDate) myAct.setActDueDate(actDueDate);
