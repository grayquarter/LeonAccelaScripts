function getInspectionsLatestStatus() {
    ...
    var itemCap = capId;
    if (arguments.length > 0 && arguments[0]) itemCap = arguments[0]; 					// use cap ID specified in args
	var required = false;
    if (arguments.length > 1 && arguments[1]) required = true;     						// "required" param = true or false. Only include required inspections?
    var excludeInspectionTypes = new Array();
    if (arguments.length > 2 && arguments[2]) excludeInspectionTypes = arguments[2];	// use exclude inspection types specified in args
	var inspectionStatuses = null;
	// var inspectionStatuses = [null, "Scheduled", "Pending"];
    if (arguments.length > 3 && arguments[3]) inspectionStatuses = arguments[3]; 		// "staus" param = result status of inspection. This determines what the status is of the inspection. In your case, not approved.
...
//		logDebug("Checking " + fMsg);
		if (required && !inspObj.getInspection().getActivity().getRequiredInspection().equals("Y")) continue;
		// {	logDebug("Skipping (Required) " + fMsg);	continue; }
        if (exists(inspObj.getInspectionType(), excludeInspectionTypes)) continue;// skip exclude inspection types
		// {	logDebug("Skipping (Type) " + fMsg);	continue; }
        if (exists(inspObj.getInspectionStatus(), ["Rescheduled", "Cancelled"])) continue; // skip rescheduled and canceled.
		// {	logDebug("Skipping (Status: Rescheduled, Cancelled) " + fMsg);  continue; } 
        if (inspectionStatuses && !exists(inspObj.getInspectionStatus(), inspectionStatuses)) continue; // only include these inspection statuses
...
//			logDebug("Adding " + fMsg);
        ...
        if (exists(inspObj.getInspectionStatus(), ["Pending","Scheduled"])) { logDebug("Replacing (Status: Pending, Scheduled) " + fMsg);  qualifiedInspections[inspectionType] = inspObj; continue; } // Replace Pending and Scheduled.
