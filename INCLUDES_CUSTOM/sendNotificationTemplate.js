function sendNotificationTemplate(contactTypes, templateName) {
	...
	if (arguments.length > 2)
		templateParams = arguments[2];
	if (arguments.length > 3)
		reportName = arguments[3];
	if (arguments.length > 4)
		reportParams = arguments[4];
	if (arguments.length > 5)
		reportModule = arguments[5];
	try {
		var emailToArray = [],
		emailCCArray = [];
		var capContacts = [];
		...
		if (contactTypes && contactTypes.indexOf("License Professional")) {
			...
			for (x in lpa) {
				if (lpa[x].getLicenseType().substring(0, 10) == "Contractor") {
					...
					if (lpName == null)
						lpName = (lpa[x].getContactFirstName() ? lpa[x].getContactFirstName() + " " : "") + (lpa[x].getContactLastName() ? lpa[x].getContactLastName() : "");
					lpPhone = lpa[x].getPhone1();
					lpEmail = lpa[x].getEmail();
					if (lpa[x].getPrintFlag() == "Y")
						break; // Use Primary if found.
				}
			}
			if (lpEmail != "")
				emailToArray.push(lpEmail);
		}

		// Get array of Contact Email Addresses based on contact type(s)
		var capContacts = [];
		if (contactTypes)
			var capContacts = getContactByTypes_TPS(contactTypes, capId);
		if (!capContacts) {
			logDebug(contactTypes + " was not found.");
			return false;
		}
		for (c in capContacts) {
			...
			if (contactEmail == null || contactEmail == "") {
				logDebug(contactType + " email was not found.");
				continue;
			}
			if (!exists(contactEmail, emailToArray))
				emailToArray.push(contactEmail);
		}
		...
		if (templateParams == null) {
			...
			if (templateName && templateName == "UPDATE INFORMATION BY CITIZEN ON RECORD NOTIFICATION")
				getACARecordActionParam4Notification(templateParams, acaUrl, 1011);
			if (templateName && templateName == "UPDATE RECEIVED ON RECORD NOTIFICATION")
				addParameter(params, "$$aaRecordUrl$$", (avUrl ? getAARecordUrl(avUrl) : ""));
		}

		var emailTo = "",
		emailCC = "",
		emailFrom = sysFromEmail;
		if (templateName) {
			var emailTemplateTo = null,
			emailTemplateCC = null,
			...
			if (!tmplResult.getSuccess()) {
				...
				if (emailTemplateFrom && emailTemplateFrom != "")
