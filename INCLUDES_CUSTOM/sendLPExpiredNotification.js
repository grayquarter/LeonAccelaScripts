	...
	for (capLic in capLicenseArr) {
		...
		if (lpEmail && lpEmail != "") {
			...
			if (applicantEmail && applicantEmail != "")
				emailCC.push(applicantEmail);
			emailCC.push(emailAddressLicensing);
		} else {
			if (applicantEmail && applicantEmail != "")
				emailTo.push(applicantEmail);
			emailTo.push(emailAddressLicensing);
			lpEmailMsg = "<p>The contractor " + lpBusName + " does not have a valid email address on file.</p>"
		}
		if (!primaryLPEmail) {
			primaryLP = getPrimaryLP(itemCap);
			if (primaryLP)
				primaryLPEmail = primaryLP.getEmail();
		}
		if (primaryLPEmail && primaryLPEmail != "" && !exist(primaryLPEmail, emailTo) && !exist(primaryLPEmail, emailCC))
			emailCC.push(primaryLPEmail);
