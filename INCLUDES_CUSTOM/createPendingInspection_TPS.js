function createPendingInspection_TPS(iGroup, iType) { // optional Cap ID
	...
	if (arguments.length > 3 && arguments[3])
		iRequired = arguments[3]; // use required inspection flag specified in args
	var requestComment = null;
	if (arguments.length > 4 && arguments[4])
		requestComment = arguments[4]; // use Request Comment specified in args
	var requestorUserID = null;
	if (arguments.length > 5 && arguments[5])
		requestorUserID = arguments[5]; // use Requestor User ID specified in args
	var requestorPhone = null;
	if (arguments.length > 6 && arguments[6])
		requestorPhone = arguments[6]; // use Requestor Phone specified in args
	...
	if (requestorUserID)
		activityModel.setRequestorUserID(requestorUserID);
	//	if (requestorUserID) activityModel.setAuditID(requestorUserID); // This does not work.
	if (requestorPhone)
		activityModel.setReqPhoneNum(requestorPhone);
	if (requestComment)
		inspModel.getRequestComment().setText(requestComment); // Update Request Comment to indicate added by Script.
