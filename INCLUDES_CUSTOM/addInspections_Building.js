function addInspections_Building() {
	...
	if (arguments.length > 0)
		inspectionsNeedUpdate = arguments[0];
	var newFeeArray = "";
	...
	inspArray = new Array();
	for (ii in inspOptArray)
		inspArray.push(inspOptArray[ii]);
	for (ii in inspReqArray)
		inspArray.push(inspReqArray[ii]);
	...
	if (inspResultObj.getSuccess()) {
		...
		for (xx in inspList) {
			...
			if (!inspList[xx].getAuditStatus().equals("A"))
				continue; // ignore inactive inspections.
			if (!exists(inspectionStatus.toUpperCase(), ["SCHEDULED", "PENDING"]))
				continue; // ignore inspections that are not Scheduled or Pending.
			if (exists(inspectionType, inspOptArray))
				continue; // ignore Optional Inspections
			if (exists(inspectionType, inspReqArray))
				continue; // ignore Required Inspections
			// Remove inspection
	...
	for (ii in inspArray) {
		...
		inspRequired = exists(inspType, inspReqArray)
			if (!isScheduled(inspType))
				createPendingInspection_TPS(inspGroup, inspType, capId, inspRequired, "Added By Script");
	}
}
