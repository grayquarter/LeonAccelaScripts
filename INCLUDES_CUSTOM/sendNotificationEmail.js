function sendNotificationEmail(emailFrom, emailTo, emailCC, templateName, templateParams, reportFile) {
	...
	if (arguments.length > 7)
		sendEmailDup = arguments[7]; // Check if email was already sent.

	if (matches(emailFrom, null, "", undefined))
		emailFrom = "";
	if (matches(emailTo, null, "", undefined))
		emailTo = "";
	if (matches(emailCC, null, undefined))
		emailCC = "";
	emailBCC = "";
	emailPriority = null;
	...
	try {
		...
		if (templateName && templateName != "") {
			...
			if (templateResult.getSuccess()) {
				...
				if (emailFrom == "" && templateModel.getFrom())
					emailFrom = templateModel.getFrom();
				if (emailTo == "" && templateModel.getTo())
					emailTo = templateModel.getTo();
				if (emailCC == "" && templateModel.getCc())
					emailCC = templateModel.getCc();
				if (emailBCC == "" && templateModel.getBcc())
					emailBCC = templateModel.getBcc();
		...
		if (matches(emailFrom, null, "", undefined))
			emailFrom = systemMailFrom;
		// Check Parameters
		var paramErrors = [];
		if (matches(templateName, null, "", undefined))
			paramErrors.push("Template Name");
		if (matches(emailFrom, null, "", undefined))
			paramErrors.push("From email address");
		if (matches(emailTo, null, "", undefined))
			paramErrors.push("TO email address");
		...
		while (keys.hasMoreElements()) {
			...
			if (keyValue != KeyValueOnly)
				templateParamsHaveHTML = true;
		...
		if (skipSendEmail) {
			logDebug("<font color=red>Skipping previously sent email:</font> " + emailTitle);
			return false;
		}

		var oMsg = "";
		if (emailTitle && emailTitle != "")
			oMsg += ", Title: " + emailTitle;
		if (emailTo && emailTo != "")
			oMsg += ", To: " + emailTo;
		if (emailCC && emailCC != "")
			oMsg += ", CC: " + emailCC;
		...
		if (templateParamsHaveHTML) { // template parameters have HTML tags that will not come across using standard method for sending notification templates
			...
			for (i in emailFromList) {
				if (emailFromList[i] != "")
					emailContacts.push([emailFromList[i], "FROM"]);
			}
			for (i in emailToList) {
				if (emailToList[i] != "")
					emailContacts.push([emailToList[i], "TO"]);
			}
			for (i in emailCCList) {
				if (emailCCList[i] != "")
					emailContacts.push([emailCCList[i], "CC"]);
			}
			for (i in emailBCCList) {
				if (emailBCCList[i] != "")
					emailContacts.push([emailBCCList[i], "BCC"]);
			}
			...
			if (emailPriority)
				emailMessageScriptModel.setPriority(emailPriority);
