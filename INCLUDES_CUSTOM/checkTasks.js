function checkTasks(stask) {
	...
	if (arguments.length > 1 && arguments[1])
		taskNames = arguments[1];
	if (arguments.length > 2 && arguments[2])
		taskStatuses = arguments[2];
	...
	for (xx in taskArr) {
		if (stask && !taskArr[xx].getProcessCode().equals(stask))
			continue; // Include only process code specified
		if (taskNames && !exists(taskArr[xx].getTaskDescription(), taskNames))
			continue; // Include tasks by name
		if (!taskArr[xx].getActiveFlag().equals("Y")) { // Exclude inactive tasks if no statuses specified or matches status specified.
			if (!taskStatuses)
				continue;
			if (exists(taskArr[xx].getDisposition(), taskStatuses))
				continue; // Exclude tasks by status
		}
