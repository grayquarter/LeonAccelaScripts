function loadFees_TPS() { // option CapId
	...
	if (arguments.length > 1 && arguments[1])
		feeStatusList = arguments[1];

	var feeArrayType = "List";
	if (arguments.length > 2 && (arguments[2], ["By Sequence", "By Code"]))
		feeArrayType = arguments[2];
	...
	for (ff in feeObjArr) {
		fFee = feeObjArr[ff];
		if (feeStatusList && !exists(fFee.getFeeitemStatus(), feeStatusList))
			continue; // Ignore if not in Fee Status List.
		var myFee = new Fee_TPS(fFee);
		if (feeArrayType == "List")
			feeArr.push(myFee);
		if (feeArrayType == "By Sequence")
			feeArr[fFee.getFeeSeqNbr()] = myFee;
		if (feeArrayType == "By Code")
			feeArr[fFee.getFeeCod()] = myFee; // Warning: Using this option may cause some fees to not be included if they are a duplicate fee code of an existing fee.
	}
