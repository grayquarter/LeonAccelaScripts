function updateComplaintCasesTableRow(tableName, tableRow, r) {
	...
	if (tableRow["Case #"] == "" || tableRow["Case #"] == "(Total)") {
		tableRow["Status"] = "(";
		logDebug("Skipped " + rMsg);
		return false;
	}
	// logDebug("Checking " + rMsg)
	rCapId = aa.cap.getCapID(tableRow["Case #"]).getOutput();
	if (rCapId == null) {
		tableRow["Status"] = "(Unknown)";
		tableRow["Comments"] += (tableRow["Comments"].indexOf("Record not found") >= 0 ? "" : "; Record Not Found");
		logDebug("Skipped " + rMsg);
		return false;
	}
	...
	if (rCapStatus == "COMPLAINT", rCapStatus == "NOV")
		rCapStatus = "Open";
	if (rCapStatus == "CEB FINDING")
		rCapStatus = "Code Enforcement Board Finding";
	if (rCapStatus == "CEB LIEN")
		rCapStatus = "Code Enforcement Board Lien";
	if (rCapStatus == "CLOSED")
		rCapStatus = "Closed";
	if (rCapStatus == "INVALID")
		rCapStatus = "Invalid";
	if (rCapStatus == "REFERRED")
		rCapStatus = "Referred";
	if (rCapStatus == "VOLUNTARY COMPLIANCE")
		rCapStatus = "Voluntary Compliance";
	if (rCapStatus == "VOID")
		rCapStatus = "Void";
