function getParams4Notification(params) {
	...
	if (itemCap != null) {
		...
		if (typeof(avUrl) != "undefined" && avUrl)
			addParameter(templateParams, "$$aaRecordUrl$$", getAARecordUrl(avUrl, itemCap));
		...
		if (capCreator)
			capCreatorEmail = getUserEmail(capCreator);
		...
		if (capAssigned)
			capAssignedStaffEmail = getUserEmail(capAssigned);
		if (!capAssignedStaffEmail)
			capAssignedStaffEmail = null;
	...
	if (typeof(inspResult) != "undefined") {
		if (typeof(inspComment) == "undefined")
			inspComment = inspResultComment;
	...
	if (!publicUser && systemUserObj) {
		...
		if (currentUserFullName == null)
			currentUserFullName = ((currentUserFirstName ? currentUserFirstName + " " : "") + (currentUserLastName ? currentUserLastName : "")).trim();
