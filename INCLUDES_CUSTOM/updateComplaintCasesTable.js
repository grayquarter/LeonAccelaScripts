function updateComplaintCasesTable(pCapType) {
	try {
		...
		for (x in capList) {
			...
			if (exists(capListId, rCapIdList))
				continue; // Cap already exists on list.
		...
		if (!tableArr)
			tableArr = new Array(); // Problem loading Array use empty array
		...
		for (var r in tableArr) {
			...
			if (tableRow["Case #"] == "" || tableRow["Case #"] == "(Total)")
				continue;
		...
		var readOnly = 'N';
		for (x in rCapIdList) {
			if (exists(rCapIdList[x], capIdList))
				continue; // Skip Cap Id that already exist in table.
			rCapId = aa.cap.getCapID(rCapIdList[x]).getOutput();
			if (rCapId == null)
				continue; // Skip Cap Id not found
			rCapIdString = rCapId.getCustomID();
			var rCap = aa.cap.getCap(rCapId).getOutput();
			if (rCap == null)
				continue; // Skip Cap not found
			...
			if (exists(rCapStatus, ["CLOSED", "INVALID", "VOID", "VOLUNTARY COMPLIANCE"]) && !(referToCEB || referToNAB)) {
				logDebug("Skipping (2)" + " status: " + rCapStatus + (referToCEB ? " referred CEB" : "") + (referToNAB ? " referred NAB" : ""));
				continue;
			}
			if (!exists(rCapStatus, ["COMPLAINT", "NOV", "CEB FINDING", "CEB LIEN"]) && !(referToCEB || referToNAB)) {
				logDebug("Skipping (1)" + " status: " + rCapStatus + (referToCEB ? " referred CEB" : "") + (referToNAB ? " referred NAB" : ""));
				continue;
			}
